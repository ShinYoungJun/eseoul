...
@Controller("file.RojumFileController")
public class RojumFileController {
	// DAO 객체 연결 과정 추가
	@Resource(name = "dao")
	...
	@RequestMapping(value = "/bbs/rojum_upload_list.action", method = { RequestMethod.GET, RequestMethod.POST })
	public String uploadlist(FileCommand command, HttpServletResponse res,
			...
			return "/jsp/rojum/bbs/register.jsp";
		}
	
	@RequestMapping(value = "/bbs/rojum_upload.action", method = { RequestMethod.GET, RequestMethod.POST })
	public String upload(FileCommand command, HttpServletResponse res,
		...
		// 리다이렉트 지정
		return "redirect:/bbs/rojum_list.action";

	}

	@RequestMapping(value = "/bbs/rojum_download.action", method = { RequestMethod.GET,
			RequestMethod.POST })
	...
	//게시판 리스트 호출
	@RequestMapping(value = "/bbs/rojum_list.action", method = { RequestMethod.GET,
			RequestMethod.POST })
	public String list(HttpServletResponse res, HttpServletRequest req) throws Exception {
		...
		//페이지 번호 문자열 재전송
		String urlList = "/bbs/rojum_list.action";
		req.setAttribute("pageIndexList", myUtil.pageIndexList(current_page, total_page, urlList)); 
		
		return "/jsp/rojum/bbs/view.jsp";
	}
	
	//내용 보는거 ㅠㅠ 아 힘들다
	@RequestMapping(value="/bbs/rojum_content.action"
	...
	public String content(FileCommand command, HttpServletResponse res, HttpServletRequest req) throws Exception {
		...
		return "/jsp/rojum/bbs/contentview.jsp";
	}
	
	//지우는거 ;;;;마지막!!
	@RequestMapping(value="/bbs/rojum_delete.action"
		, method={RequestMethod.GET, RequestMethod.POST})
	...
	//수정한 게시물 업데이트
	@RequestMapping(value="/bbs/rojum_update.action"
			, method={RequestMethod.GET, RequestMethod.POST})
	public String update(FileCommand command, HttpServletResponse res, HttpServletRequest req) throws Exception {
		if (command == null || command.getMode() == null || command.getMode().equals("")) {
			...
			req.setAttribute("dto", dto);
			return "/jsp/rojum/bbs/modify.jsp";
		}
		Map<String, Object> hMap = new HashMap<String, Object>();
		if (!command.getUpload().isEmpty()) {
			HttpSession session = req.getSession();
	...
	//게시물 검색 기능
	@RequestMapping(value = "/bbs/rojum_search.action", method = { RequestMethod.GET,
			RequestMethod.POST })
	public String search(FileCommand command, HttpServletResponse res, HttpServletRequest req) throws Exception {
		...
		req.setAttribute("lists", lists);
			...
			//페이지 번호 문자열 재전송
			String urlList = "/bbs/rojum_search.action?name="+name+"&title="+title+"&";
			req.setAttribute("pageIndexList", myUtil.pageIndexList(current_page, total_page, urlList));
	...
	/*연락처 영역*/
	//연락처 리스트 호출
	@RequestMapping(value = "/bbs/rojum_adlist.action", method = { RequestMethod.GET,
			RequestMethod.POST })
	public String adlist(HttpServletResponse res, HttpServletRequest req) throws Exception {	
		...
		//페이지 번호 문자열 재전송
		String urlList = "/bbs/rojum_adlist.action";
		req.setAttribute("pageIndexList", myUtil.pageIndexList(current_page, total_page, urlList));
		
		return "/jsp/rojum/adress/view.jsp";
	}
	
	@RequestMapping(value = "/bbs/rojum_adupload.action", method = { RequestMethod.GET, RequestMethod.POST })
	public String adupload(FileCommand command, HttpServletResponse res,
			HttpServletRequest req) throws Exception {
		if (command == null || command.getMode() == null || command.getMode().equals("")) {
			return "/jsp/rojum/adress/register.jsp";
		}
		// 새 글번호 얻는 부분 추가
		int div = 4;
		...
		// 리다이렉트 지정
		return "redirect:/bbs/rojum_adlist.action";

	}

	//게시물 삭제
	@RequestMapping(value="/bbs/rojum_addelete.action"
		, method={RequestMethod.GET, RequestMethod.POST})
	...
	//연락처 검색 기능
	@RequestMapping(value = "/bbs/rojum_adsearch.action", method = { RequestMethod.GET,
			RequestMethod.POST })
	public String adsearch(FileCommand command, HttpServletResponse res, HttpServletRequest req) throws Exception {
		...
		req.setAttribute("adlists", adlists);
			...
			//페이지 번호 문자열 재전송
			String urlList = "/bbs/rojum_adsearch.action?gu_nm="+gu+"&or_nm="+or+"&t_nm="+te+"&dam_nm="+dam+"&";
			req.setAttribute("pageIndexList", myUtil.pageIndexList(current_page, total_page, urlList));
			
			return "/jsp/rojum/adress/view.jsp";
		}
	//수정할 연락처 업데이트
	@RequestMapping(value="/bbs/rojum_adupdate.action"
			, method={RequestMethod.GET, RequestMethod.POST})
	public String adupdate(FileCommand command, HttpServletResponse res, HttpServletRequest req) throws Exception {
		if (command == null || command.getMode() == null || command.getMode().equals("")) {
			...
			req.setAttribute("dto", dto);
			return "/jsp/rojum/adress/modify.jsp";
		}
			Map<String, Object> hMap = new HashMap<String, Object>();
			hMap.put("sid", command.getSid());
			hMap.put("gu_nm", command.getGu_nm());
