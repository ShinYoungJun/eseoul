...
@Controller("file.GapanFileController")
public class GapanFileController {
	// DAO 객체 연결 과정 추가
	@Resource(name = "dao")
	...
	@RequestMapping(value = "/bbs/gapan_upload_list.action", method = { RequestMethod.GET, RequestMethod.POST })
	public String uploadlist(FileCommand command, HttpServletResponse res,
			...
			return "/jsp/gapan/bbs/register.jsp";
		}
	
	@RequestMapping(value = "/bbs/gapan_upload.action", method = { RequestMethod.GET, RequestMethod.POST })
	public String upload(FileCommand command, HttpServletResponse res,
		...
		// 리다이렉트 지정
		return "redirect:/bbs/gapan_list.action";

	}

	@RequestMapping(value = "/bbs/gapan_download.action", method = { RequestMethod.GET,
			RequestMethod.POST })
	...
	//게시판 리스트 호출
	@RequestMapping(value = "/bbs/gapan_list.action", method = { RequestMethod.GET,
			RequestMethod.POST })
	public String list(HttpServletResponse res, HttpServletRequest req) throws Exception {
		...
		//페이지 번호 문자열 재전송
		String urlList = "/bbs/gapan_list.action";
		req.setAttribute("pageIndexList", myUtil.pageIndexList(current_page, total_page, urlList)); 
		
		return "/jsp/gapan/bbs/view.jsp";
	}
	
	//게시물 내용
	@RequestMapping(value="/bbs/gapan_content.action"
	...
	public String content(FileCommand command, HttpServletResponse res, HttpServletRequest req) throws Exception {
		...
		return "/jsp/gapan/bbs/contentview.jsp";
	}
	
	//게시물 삭제
	@RequestMapping(value="/bbs/gapan_delete.action"
		, method={RequestMethod.GET, RequestMethod.POST})
	...
	//수정한 게시물 업데이트
	@RequestMapping(value="/bbs/gapan_update.action"
			, method={RequestMethod.GET, RequestMethod.POST})
	public String update(FileCommand command, HttpServletResponse res, HttpServletRequest req) throws Exception {
		if (command == null || command.getMode() == null || command.getMode().equals("")) {
			...
			req.setAttribute("dto", dto);
			return "/jsp/gapan/bbs/modify.jsp";
		}
		Map<String, Object> hMap = new HashMap<String, Object>();
		if (!command.getUpload().isEmpty()) {
			HttpSession session = req.getSession();
	...
	//게시물 검색 기능
	@RequestMapping(value = "/bbs/gapan_search.action", method = { RequestMethod.GET,
			RequestMethod.POST })
	public String search(FileCommand command, HttpServletResponse res, HttpServletRequest req) throws Exception {
		...
		req.setAttribute("lists", lists);
			...
			//페이지 번호 문자열 재전송
			String urlList = "/bbs/gapan_search.action?name="+name+"&title="+title+"&";
			req.setAttribute("pageIndexList", myUtil.pageIndexList(current_page, total_page, urlList));
	...
	/*연락처 영역*/
	//연락처 리스트 호출
	@RequestMapping(value = "/bbs/gapan_adlist.action", method = { RequestMethod.GET,
			RequestMethod.POST })
	public String adlist(HttpServletResponse res, HttpServletRequest req) throws Exception {	
		...
		//페이지 번호 문자열 재전송
		String urlList = "/bbs/gapan_adlist.action";
		req.setAttribute("pageIndexList", myUtil.pageIndexList(current_page, total_page, urlList));
		
		return "/jsp/gapan/adress/view.jsp";
	}
	
	@RequestMapping(value = "/bbs/gapan_adupload.action", method = { RequestMethod.GET, RequestMethod.POST })
	public String adupload(FileCommand command, HttpServletResponse res,
			HttpServletRequest req) throws Exception {
		if (command == null || command.getMode() == null || command.getMode().equals("")) {
			return "/jsp/gapan/adress/register.jsp";
		}
		// 새 글번호 얻는 부분 추가
		int div = 3;
		...
		// 리다이렉트 지정
		return "redirect:/bbs/gapan_adlist.action";

	}

	//지우는거 ;;;;마지막!!
	@RequestMapping(value="/bbs/gapan_addelete.action"
		, method={RequestMethod.GET, RequestMethod.POST})
	...
	//연락처 검색 기능
	@RequestMapping(value = "/bbs/gapan_adsearch.action", method = { RequestMethod.GET,
			RequestMethod.POST })
	public String adsearch(FileCommand command, HttpServletResponse res, HttpServletRequest req) throws Exception {
		...
		req.setAttribute("adlists", adlists);
			...
			//페이지 번호 문자열 재전송
			String urlList = "/bbs/gapan_adsearch.action?gu_nm="+gu+"&or_nm="+or+"&t_nm="+te+"&dam_nm="+dam+"&";
			req.setAttribute("pageIndexList", myUtil.pageIndexList(current_page, total_page, urlList));
			
			return "/jsp/gapan/adress/view.jsp";
		}
	//수정할 연락처 업데이트
	@RequestMapping(value="/bbs/gapan_adupdate.action"
			, method={RequestMethod.GET, RequestMethod.POST})
	public String adupdate(FileCommand command, HttpServletResponse res, HttpServletRequest req) throws Exception {
		if (command == null || command.getMode() == null || command.getMode().equals("")) {
			...
			req.setAttribute("dto", dto);
			return "/jsp/gapan/adress/modify.jsp";
		}
			Map<String, Object> hMap = new HashMap<String, Object>();
			hMap.put("sid", command.getSid());
			hMap.put("gu_nm", command.getGu_nm());
