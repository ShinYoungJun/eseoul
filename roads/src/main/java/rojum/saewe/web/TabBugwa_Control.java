...
//		 삭제 세션
		...
		try{
			if(userId != null)
			{
				...
				System.out.println("bugwa View :: " + ADMIN_NO);
				System.out.println("bugwa View :: " + SEQ);
				
				List		list 	= iSaeweService.executeBugwa_View_GpTyp(ADMIN_NO, SEQ);
				...
				if(SEQ == null	||	SEQ.trim().equals("")){	//	산출정보가 하나도 없을경우 빈화면 출력
					return new ModelAndView("/rojum/saewe/tab/bugwa_view_sample");
				...
				else
				{
					...
					List	data	= iSaeweService.executeBugwa_DataInfo_GpTyp(ADMIN_NO, SEQ);
...
//					request.setAttribute("BONSE", saeweBean.getSUM_ADJUST());
		...
		return new ModelAndView("/rojum/saewe/tab/bugwa_view", "board", Bean);
	}
	...
	public ModelAndView 	Bugwa_Modify(HttpServletRequest request, HttpServletResponse response) throws Exception 
	{
		...
		return new ModelAndView("/rojum/saewe/tab/bugwa_modify", "board", Bean);
	}
...
//		 삭제 세션
		...
		try{
			if(userId != null)
			{
				...
				{
					return new ModelAndView("/rojum/saewe/tab/bugwa_register");
				}
