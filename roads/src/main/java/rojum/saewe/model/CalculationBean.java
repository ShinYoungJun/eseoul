
package main.java.rojum.saewe.model;

public class CalculationBean {

	String	ADMIN_NO		= "";
	String	YEAR			= "";
	String	SEQ				= "";
	String	PRICE			= "";
	String	AREA_BEFORE		= "";
	String	AREA_AFTER		= "";
	String	RATE			= "";
	String	SUM_LASTYEAR	= "";
	String	SUM_YEAR		= "";
	String	SUM_ADJUST		= "";
	String	TAX				= "";
	String	FROMDATE		= "";
	String	TODATE			= "";
	String	FORM_ADJUST		= "";
	String	FORM			= "";
	String	UP_DATE			= "";
	String	REASON			= "";
	String	LINCESE_TAX		= "";
	String	EARNING			= "";
	String	NOTES			= "";	
	String	AMOUNT			= "";
	String	ADJUSTMENT		= "";
	
	String	SANCHUL_DATE	= "";	//	산출기간

	public String getADMIN_NO() {
		return ADMIN_NO;
	}

	public void setADMIN_NO(String admin_no) {
		ADMIN_NO = admin_no;
	}

	public String getYEAR() {
		return YEAR;
	}

	public void setYEAR(String year) {
		YEAR = year;
	}

	public String getSEQ() {
		return SEQ;
	}

	public void setSEQ(String seq) {
		SEQ = seq;
	}

	public String getPRICE() {
		return PRICE;
	}

	public void setPRICE(String price) {
		PRICE = price;
	}

	public String getAREA_BEFORE() {
		return AREA_BEFORE;
	}

	public void setAREA_BEFORE(String area_before) {
		AREA_BEFORE = area_before;
	}

	public String getAREA_AFTER() {
		return AREA_AFTER;
	}

	public void setAREA_AFTER(String area_after) {
		AREA_AFTER = area_after;
	}

	public String getRATE() {
		return RATE;
	}

	public void setRATE(String rate) {
		RATE = rate;
	}

	public String getSUM_LASTYEAR() {
		return SUM_LASTYEAR;
	}

	public void setSUM_LASTYEAR(String sum_lastyear) {
		SUM_LASTYEAR = sum_lastyear;
	}

	public String getSUM_YEAR() {
		return SUM_YEAR;
	}

	public void setSUM_YEAR(String sum_year) {
		SUM_YEAR = sum_year;
	}

	public String getSUM_ADJUST() {
		return SUM_ADJUST;
	}

	public void setSUM_ADJUST(String sum_adjust) {
		SUM_ADJUST = sum_adjust;
	}

	public String getTAX() {
		return TAX;
	}

	public void setTAX(String tax) {
		TAX = tax;
	}

	public String getFROMDATE() {
		return FROMDATE;
	}

	public void setFROMDATE(String fromdate) {
		FROMDATE = fromdate;
	}

	public String getTODATE() {
		return TODATE;
	}

	public void setTODATE(String todate) {
		TODATE = todate;
	}

	public String getFORM_ADJUST() {
		return FORM_ADJUST;
	}

	public void setFORM_ADJUST(String form_adjust) {
		FORM_ADJUST = form_adjust;
	}

	public String getFORM() {
		return FORM;
	}

	public void setFORM(String form) {
		FORM = form;
	}

	public String getUP_DATE() {
		return UP_DATE;
	}

	public void setUP_DATE(String up_date) {
		UP_DATE = up_date;
	}

	public String getREASON() {
		return REASON;
	}

	public void setREASON(String reason) {
		REASON = reason;
	}

	public String getLINCESE_TAX() {
		return LINCESE_TAX;
	}

	public void setLINCESE_TAX(String lincese_tax) {
		LINCESE_TAX = lincese_tax;
	}

	public String getEARNING() {
		return EARNING;
	}

	public void setEARNING(String earning) {
		EARNING = earning;
	}

	public String getNOTES() {
		return NOTES;
	}

	public void setNOTES(String notes) {
		NOTES = notes;
	}

	public String getAMOUNT() {
		return AMOUNT;
	}

	public void setAMOUNT(String amount) {
		AMOUNT = amount;
	}

	public String getADJUSTMENT() {
		return ADJUSTMENT;
	}

	public void setADJUSTMENT(String adjustment) {
		ADJUSTMENT = adjustment;
	}

	public String getSANCHUL_DATE() {
		return SANCHUL_DATE;
	}

	public void setSANCHUL_DATE(String sanchul_date) {
		SANCHUL_DATE = sanchul_date;
	}
}
