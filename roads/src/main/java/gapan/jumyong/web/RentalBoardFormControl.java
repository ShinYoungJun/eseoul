...
public class RentalBoardFormControl extends SimpleFormController {	
	...
	protected Object formBackingObject(HttpServletRequest request) throws ServletException {
		...
		System.out.println("rentalBoardFormControl : : formBackingObject");
		...
		try{
			...
			if(userId != null)
			{
				...
				List productList = objJumyongService.ProductList(Integer.parseInt(strCurrentPage),5,gapanNo,"4");
	...
	//			리스트의 페이징 html
				...
				int totalProductCount = objJumyongService.getProductListCount(gapanNo,"4");
				...
				String strFormView = "/gapan/jumyong/info_rental";
				setFormView(strFormView);
					...
					if(mode.equals("add"))		//add 일때 요율 구하기
					{
						...
						List leep = objJumyongService.getCalculationDate(gapanNo, "4");
						
						Map retVal = objJumyongService.getInitCal(gapanNo,"4");
						...
						if(retVal.get("SUM_LASTYEAR") != null){
							if(intTodate == 1 && intFromdate == 1){
								...
								List list = objJumyongService.getCalculationDate(gapanNo,"4");
						...
						if(retVal.get("SF_VALUE") != null){
							sf_value = retVal.get("SF_VALUE").toString();
						}else{
							sf_value = "0";
						}
				...
				setSuccessView("redirect:/gapan/jumyong/rentalInsert.do?GAPAN_NO="+gapanNo);
				...
				request.setAttribute("taxcode", commonCode.executeTaxCode("10", pb.getTAX_CD(), "선택"));//	세목코드
				...
				System.out.println(""+userBean.getDEPT_CD()+"::"+userBean.getORG_CD());
	...
	protected ModelAndView onSubmit(HttpServletRequest request, HttpServletResponse response, Object command, BindException errors) throws Exception {
		...
		System.out.println("rentalBoardFormControl : : onSubmit"+getSuccessView().toString());
			...
			if(mode.equals("add")){
				...
				for(int i =0; i<leep.size();i++){
					...
					if(Integer.parseInt(bb.getFROMDATE())<=from && Integer.parseInt(bb.getTODATE())>=from){
						...
						return new ModelAndView(getSuccessView()+"&message=산출기간이 중복되었습니다.");
				...
				long overtax = (long)(Double.parseDouble(objProductBean.getSUM_ADJUST())/100*Double.parseDouble(objProductBean.getGasanRate())+Double.parseDouble(objProductBean.getSUM_ADJUST()));
