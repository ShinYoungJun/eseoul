...
//    	
//    	String sql = getMessageSourceAccessor().getMessage("Querys.jumyong.jumyonginfo.Minwon_Count");
...
//    }
	...
	{
    	JumyonginfoMapDao.Jumin_Modify	jumin_Modify	= jumyonginfoMapDao.new Jumin_Modify(getDataSource(), getMessageSourceAccessor());
	...
	{
    	JumyonginfoMapDao.Jumji_View2	jumji_View2	= jumyonginfoMapDao.new Jumji_View2(getDataSource(), getMessageSourceAccessor());
...
//    	System.out.println("###########     executeJumji_View       #############");
...
//    	System.out.println("#####################################################");
    ...
    public	boolean	executeJumji_Modify(PlaceBean Bean)
	{
		...
		try
		{
			System.out.println("####################### (jumyonginfoDao) executeJumji_Modify  ######################################");
	  ...
	  {
	  	Object[] values = {corp_cd, gu_code, admin_no};
	  	
	  	String sql = getMessageSourceAccessor().getMessage("Querys.facility.jumyonginfo.Jumji_Count");
			return 	getJdbcTemplate().queryForInt(sql, values);
	  }
	  ...
	  //	점용지 정보 등록
		...
		{
	    	JumyonginfoMapDao.Jumji_Register	jumji_Register	= jumyonginfoMapDao.new Jumji_Register(getDataSource(), getMessageSourceAccessor());
			try
			{
				System.out.println("***********************faculty jumyonginfoDao executeJumji_Register.()+++++++++++");
				System.out.println("Bean.getCORP_CD() ==>"+Bean.getCORP_CD());
				System.out.println("Bean.getGU_CODE() ==>"+Bean.getGU_CODE());
				System.out.println("Bean.getREQUEST_NO() ==>"+Bean.getREQUEST_NO());
				System.out.println("Bean.getOBJECT_NO() ==>"+Bean.getOBJECT_NO());
...
//						Bean.getSISUL_ADDR(),
	    ...
	    public	String	getObject_No(PlaceBean Bean)
		{
	    	...
	    	Object[] values = {Bean.getCORP_CD(), Bean.getGU_CODE(), Bean.getYEAR(), Bean.getADMIN_NO()};

	    	//String sql = getMessageSourceAccessor().getMessage("Querys.facility.minwon.getObject_No");
	    	String sql = getMessageSourceAccessor().getMessage("Querys.facility.jumyonginfo.getObject_No");
