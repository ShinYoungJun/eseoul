...
public class ProductBoardBeforeTaxFormControl_facility extends SimpleFormController {	
	...
	public ProductBoardBeforeTaxFormControl_facility() {
	...
	throws ServletException {
		...
		/*	epb = objJumyongService.Equip_GetCalSumSeq(gu_code,corp_cd,admin_no,year);
			...
			List placeList = objJumyongService.placeList(gu_code,corp_cd,admin_no,Integer.parseInt(strCurrentPage));
			
			setSuccessView("redirect:/facility/jumyong/product_view.do?GU_CODE="+gu_code+"&CORP_CD="+corp_cd+"&ADMIN_NO="+admin_no);
			...
			String strFormView = "/facility/jumyong/tab/info_product_sub_beforeTax";
	...
	protected ModelAndView onSubmit(HttpServletRequest request
		...
		{	
			if(cal_sum_seq == 0)	//cal_sum_info 최초 입력인지 확인 seq가 0이면 최초 입력,cal_sum_info에 먼저 입력후 cal_sum_info의 seq를 calculation의 cal_sum_seq에 넣음
			{
				...
				objProductBean.setSUM(objProductBean.getSUM_ADJUST() * (100 - objProductBean.getREDUCTION_RATE())/100);//최종점용료 입력
