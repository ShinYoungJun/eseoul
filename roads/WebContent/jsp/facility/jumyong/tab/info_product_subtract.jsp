...
//		document.getElementById("dateCount").value = dateCount;
	...
	function Calculate()
	{
		...
		var old_quantity = "${old_quantity}";
		var old_period = "${old_period}";
		var old_sum_adjust = "${old_sum_adjust}";
		
		
		//점용료 계산
		var result = Math.floor((Number(quantity)/Number(old_quantity)*(Number(old_period) - Number(period))/Number(old_period)*Number(old_sum_adjust)));
		...
		//산출식
		document.getElementById("FORM_ADJUST").value = old_sum_adjust+" * ( ("+old_period+" - "+period+")개월/"+old_period +"개월) * (-"+quantity+"㎡/"+old_quantity+"㎡)";
...
<form name="productForm" id="productForm" method="post" action = "/facility/jumyong/productSubtractInsert.do" >
...
<spring:bind path="product.FORM_ADJUST">                                                            
	<input name="FORM" id="FORM" type="hidden" value="${product.FORM}">
</spring:bind>

<spring:bind path="product.SUM_YEAR">                                                            
	<input name="SUM_YEAR" id="SUM_YEAR" type="hidden" value="${product.SUM_YEAR}">
</spring:bind>

<spring:bind path="product.SUM_LASTYEAR">                                                            
	<input name="SUM_LAST_YEAR" id="SUM_LASTYEAR" type="hidden" value="${product.SUM_LASTYEAR}">
</spring:bind>

<spring:bind path="product.ADJUSTMENT">                                                            
	<input name="ADJUSTMENT" id="ADJUSTMENT" type="hidden" value="${product.ADJUSTMENT}">
</spring:bind>
					...
					<table width="100%" border="1" bordercolor="#c9dfed" style="border-collapse:collapse;" cellpadding="0" cellspacing="0"  class="sub_table">
					                   <tr>
					                    ...
					                    <td  bgcolor="eaeaea" class="table_bl_left" >
										  ...
										  <td width="13%" height="26" align="center" bgcolor="e5eff8" class="sub_table_b">시작 날짜 ~<br> 종료 날짜</td>
							                    <td width="37%" bgcolor="eaeaea" class="table_bl_left" >${fromdate} ~ ${todate}
												</td>   
									...
									<tr>
				                     ...
				                     <tr>
					                  <td height="26" align="center" bgcolor="e5eff8" class="sub_table_b">기존 수량</td>
					                    <td bgcolor="eaeaea" class="table_bl_left" >${old_quantity}㎡
										</td>    
										
										<td height="26" align="center" bgcolor="e5eff8" class="sub_table_b">변경 수량</td>
					                    <td bgcolor="eaeaea" class="table_bl_left" >
					                        <spring:bind path="product.AREA_AFTER">
						                    <input name="QUANTITY" id="QUANTITY" type="text" class="input_form1" style="width:80px;ime-mode:disabled;" value="${product.QUANTITY}"   onKeyDown="onlyNumberInput();" style="text-align:right">㎡
											</spring:bind>
										</td>   
					                  </tr>
